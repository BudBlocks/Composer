...
async function sendNote(trade) {
    ...
    if (sender.earliest_note.send_date.getTime() < new Date().getTime()) {
        let factory = getFactory();
    ...
    //create the new note
    var new_note = factory.newResource('org.budblocks', 'Note', sender.ID.concat(this_note.toString()));
    ...
    participantRegistry = await getParticipantRegistry('org.budblocks.Consumer');
