...
async function addBalance(trade) {
    ...
    var recipient = await participantRegistry.get(trade.recipient);
...
async function sendNote(trade) {
    let participantRegistry = await getParticipantRegistry('org.budblocks.Consumer');
    var sender = await participantRegistry.get(trade.sender);
    var recipient = await participantRegistry.get(trade.recipient);
    ...
    var this_note = sender.notes_sent++;
    var factory = getFactory();

    //create the new note
    var new_note = factory.newResource('org.budblocks', 'Note', sender.ID.concat('.').concat(this_note.toString()));
...
async function resolveNote(trade) {
    let assetRegistry = await getAssetRegistry('org.budblocks.Note');
    var note = await assetRegistry.get(trade.note);
    let participantRegistry = await getParticipantRegistry('org.budblocks.Consumer');
    var sender = participantRegistry.get(note.sender);
    var recipient = participantRegistry.get(note.recipient);
